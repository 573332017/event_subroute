...
fig, ax = plt.subplots(dpi=150, figsize=(12, 12))
# 统计该子图下每个实体出现的数量
zitu_entity = {}
# 子图下某个实体映射成的数字
ys_en = {}
...
def draw_lines_from_file(path,col):
    ...
    points = []
    # label_list = []
    # first_time = int(ys_Time[2][0:10])
    # last_time = int(ys_Time[len(ys_Time)][0:10])
    # for i in range(first_time, last_time + 1):
    #     label_list.append(i)
    # # label_list = ["2020", "2021", "2022", "2023"]
    # plt.xticks(range(0, len(label_list) - 1, 1), label_list)
    ...
    if (fan_entity[event_id] not in y_list):
        ...
        if (num <= 1e7):
            ...
            for i in edges[event_id]:
                tmp_en = ys_en[i[0]]
                if (fan_entity[i[0]] not in y_list):
    ...
    with open(in_file,'r', encoding='utf-8') as file:
        for line in file:
            ...
            if (entity.get(elements[3]) == None):
                ...
                en_num += 1
            triple.append([entity[elements[0]], entity[elements[3]], elements[5][:10]])
